...
DECLARE_CYCLE_STAT(TEXT("FSimpleDelegateGraphTask.DeserializeObject"), STAT_FSimpleDelegateGraphTask_DeserializeObject, STATGROUP_TaskGraphTasks);
...
#define GET_STRUCT_NAME_CHECKED(StructName) ((void)sizeof(F##StructName), TEXT(#StructName))
...
DECLARE_DYNAMIC_DELEGATE_OneParam(FSavior_FinishRespawn, const bool, Success);
...
class RTS_SURVIVAL_API USaviorSettings : public UObject
{
	GENERATED_BODY()
	//
	USaviorSettings();
...
public:
	...
	//
	/* If enabled, Savior will evaluate the world hierarchy for each dynamic actor that has been respawned from data, to prevent duplicated instances.
	However, this accuracy check can cost a lot of performance cycles. Consider leaving this option disabled unless you really need it. */
	UPROPERTY(Category = "Performance", config, EditDefaultsOnly, BlueprintReadOnly)
	bool AccurateDynamicRespawnChecks;
	//
	/** Determines Auto-Instance behavior for Classes in World.
	...
	//
	/** Determines Auto-Respawn behavior for Classes in World.
...
class RTS_SURVIVAL_API UAutoInstanced : public UObject, public ISAVIOR_Serializable
{
	GENERATED_BODY()
};

UCLASS(ClassGroup = Synaptech, Category = "Synaptech", BlueprintType, meta = (DisplayName = "Savior"))
class RTS_SURVIVAL_API USavior : public UObject, public Reflector
{
	GENERATED_BODY()
	//
	...
	//
	friend class TASK_RespawnActors;
...
public:
	const FSlotMeta& GetSlotMeta() const
	{
		return SlotMeta;
	}
	const FSlotData& GetSlotData() const
	{
		return SlotData;
	}
	//
	void SetSlotMeta(const FSlotMeta& Meta)
	{
		SlotMeta = Meta;
	}
	void SetSlotData(const FSlotData& Data)
	{
		SlotData = Data;
	}
...
public:
	...
	void GetDataLayers(TArray<const UDataLayerInstance*>& OutLayers) const;
...
public:
	...
	If you need absolute control of when saving and loading starts and finishes, run the Save-Game in Synchronous Mode
	(locks Game Thread when loading). */
	...
	// UPROPERTY(Category="Savior", EditAnywhere, BlueprintReadWrite)
	// ERecordType Compression_DEPRECATED;
	//
	...
	//
	/** Blueprint Assignable Event: On Finish Respawn. */
	UPROPERTY(Category = "Events", BlueprintReadOnly, meta = (DisplayName = "[SAVIOR] On Finish Respawn:"))
	FSavior_FinishRespawn EVENT_OnFinishRespawn;
	///
	/// :: FUNCTIONS ::
	///
	...
	//
	//
	/** Saves to Slot all dynamic Parameters of Target Mutable Object Instance.
	This is NOT a call to 'Write Slot to File' function, call it manually.
	@Object: Object Reference Properties are read from. */
	UFUNCTION(Category = "Savior", BlueprintCallable, meta = (DisplayName = "[SAVIOR] Save Mutable Object", ExpandEnumAsExecs = "Result"))
	void SaveMutableObject(UCustomizableObjectInstance* Mutable, ESaviorResult& Result);
	//
	/** Loads from Slot all Properties marked 'SaveGame' of Target Object.
	This is NOT a call to 'Read Slot from File' function, call it manually.
	@Object: Object Reference loaded Properties are written to. */
	UFUNCTION(Category = "Savior", BlueprintCallable, meta = (DisplayName = "[SAVIOR] Load Mutable Object", ExpandEnumAsExecs = "Result"))
	void LoadMutableObject(UCustomizableObjectInstance* Mutable, ESaviorResult& Result);
	//
	//
	...
	//
	/** Loads from Slot all Properties of Target Actor's Dynamic Material Instances.
	This is NOT a call to 'Read Slot from File' function, call it manually.
	...
	UFUNCTION(Category = "Savior", BlueprintCallable, meta = (DisplayName = "[SAVIOR] Load Game World", WorldContext = "Context", AdvancedDisplay = "Absolute, Options", ExpandEnumAsExecs = "Result"))
	void LoadGameWorld(UObject* Context, const bool ResetLevelOnLoad, bool Absolute, FString Options, ESaviorResult& Result);
	//
	//
	...
	//
	//
	/** Saves World Data Layers' states to Slot.
	This is NOT a call to 'Write Slot to File' function, call it manually. */
	UFUNCTION(Category = "Savior", BlueprintCallable, meta = (DisplayName = "[SAVIOR] Save World Layers", WorldContext = "Context", ExpandEnumAsExecs = "Result"))
	void SaveWorldLayers(UObject* Context, ESaviorResult& Result);
	//
	/** Loads World Data Layers' states from Slot.
	This is NOT a call to 'Read Slot from File' function, call it manually. */
	UFUNCTION(Category = "Savior", BlueprintCallable, meta = (DisplayName = "[SAVIOR] Load World Layers", WorldContext = "Context", ExpandEnumAsExecs = "Result"))
	void LoadWorldLayers(UObject* Context, ESaviorResult& Result);
	//
	//
	/** Saves Game Feature Plugins' states to Slot.
	This is NOT a call to 'Write Slot to File' function, call it manually. */
	UFUNCTION(Category = "Savior", BlueprintCallable, meta = (DisplayName = "[SAVIOR] Save Game Features", WorldContext = "Context", ExpandEnumAsExecs = "Result"))
	void SaveGameFeatures(UObject* Context, ESaviorResult& Result);
	//
	/** Loads Game Feature Plugins' states from Slot.
	This is NOT a call to 'Read Slot from File' function, call it manually. */
	UFUNCTION(Category = "Savior", BlueprintCallable, meta = (DisplayName = "[SAVIOR] Load Game Features", WorldContext = "Context", ExpandEnumAsExecs = "Result"))
	void LoadGameFeatures(UObject* Context, ESaviorResult& Result);
	//
	//
	/** Generates Object Data-Record to be stored in this Slot.
	@Object: Target Object for Code Reflection Analysis. */
	UFUNCTION(Category = "Savior", meta = (DisplayName = "[SAVIOR] Generate Record : OBJECT", BlueprintInternalUseOnly = true))
	FSaviorRecord GenerateRecord_Object(const UObject* Object);
	//
	/** Generates Object Data-Record to be stored in this Slot.
	@Object: Target Object for Code Reflection Analysis. */
	UFUNCTION(Category = "Savior", meta = (DisplayName = "[SAVIOR] Generate Record : MUTABLE", BlueprintInternalUseOnly = true))
	FMutableRecord GenerateRecord_Mutable(const UCustomizableObjectInstance* Mutable);
	//
	/** Generates Component Data-Record to be stored in this Slot.
	@Component: Target Component for Code Reflection Analysis. */
	UFUNCTION(Category = "Savior", meta = (DisplayName = "[SAVIOR] Generate Record : COMPONENT", BlueprintInternalUseOnly = true))
	FSaviorRecord GenerateRecord_Component(const UActorComponent* Component);
	//
	/** Generates AI Data-Record to be stored in this Slot.
	@Component: Target Component for Code Reflection Analysis. */
	UFUNCTION(Category = "Savior", meta = (DisplayName = "[SAVIOR] Generate Record : BLACKBOARD", BlueprintInternalUseOnly = true))
	FSaviorRecord GenerateRecord_Blackboard(const UBlackboardComponent* Component, const UBlackboardData* Blackboard);
	//
	/** Generates Chaos Destruction data to be stored in this Slot.
	@Component: Target Component for Code Reflection Analysis. */
	UFUNCTION(Category = "Savior", meta = (DisplayName = "[SAVIOR] Generate Record : CHAOS GEOMETRY", BlueprintInternalUseOnly = true))
	FChaosGeometryRecord GenerateRecord_ChaosGeometry(const UGeometryCollectionComponent* Component);
	//
	/** Generates Actor Data-Record to be stored in this Slot.
	@Actor: Target Actor for Code Reflection Analysis. */
	UFUNCTION(Category = "Savior", meta = (DisplayName = "[SAVIOR] Generate Record : ACTOR", BlueprintInternalUseOnly = true))
	FSaviorRecord GenerateRecord_Actor(const AActor* Actor);
	//
	//
	/** Unpacks Object Data-Record from this Slot.
	@Object: Target Object for Code Reflection Analysis. */
	UFUNCTION(Category = "Savior", meta = (DisplayName = "[SAVIOR] Unpack Record : OBJECT", BlueprintInternalUseOnly = true, ExpandEnumAsExecs = "Result"))
	void UnpackRecord_Object(const FSaviorRecord& Record, UObject* Object, ESaviorResult& Result);
	//
	/** Unpacks Mutable Object Data-Record from this Slot.
	@Mutable: Target Object for Code Reflection Analysis. */
	UFUNCTION(Category = "Savior", meta = (DisplayName = "[SAVIOR] Unpack Record : MUTABLE", BlueprintInternalUseOnly = true, ExpandEnumAsExecs = "Result"))
	void UnpackRecord_Mutable(const FMutableRecord& Record, UCustomizableObjectInstance* Mutable, ESaviorResult& Result);
	//
	/** Unpacks Component Data-Record from this Slot.
	@Component: Target Component for Code Reflection Analysis. */
	UFUNCTION(Category = "Savior", meta = (DisplayName = "[SAVIOR] Unpack Record : COMPONENT", BlueprintInternalUseOnly = true, ExpandEnumAsExecs = "Result"))
	void UnpackRecord_Component(const FSaviorRecord& Record, UActorComponent* Component, ESaviorResult& Result);
	//
	/** Unpacks AI Data-Record from this Slot.
	@Blackboard: Target Object for Code Reflection Analysis. */
	UFUNCTION(Category = "Savior", meta = (DisplayName = "[SAVIOR] Unpack Record : BLACKBOARD", BlueprintInternalUseOnly = true, ExpandEnumAsExecs = "Result"))
	void UnpackRecord_Blackboard(const FSaviorRecord& Record, UBlackboardComponent* Component, UBlackboardData* Blackboard, ESaviorResult& Result);
	//
	/** Unpacks Chaos Geometry data from this Slot.
	@Component: Target Object for Code Reflection Analysis. */
	UFUNCTION(Category = "Savior", meta = (DisplayName = "[SAVIOR] Unpack Record : CHAOS GEOMETRY", BlueprintInternalUseOnly = true, ExpandEnumAsExecs = "Result"))
	void UnpackRecord_ChaosGeometry(const FChaosGeometryRecord& Record, UGeometryCollectionComponent* Component, ESaviorResult& Result);
	//
	/** Unpacks Actor Data-Record from this Slot.
	@Actor: Target Actor for Code Reflection Analysis. */
	UFUNCTION(Category = "Savior", meta = (DisplayName = "[SAVIOR] Unpack Record : ACTOR", BlueprintInternalUseOnly = true, ExpandEnumAsExecs = "Result"))
	...
	//
	/** Checks if current map is partitioned. */
	UFUNCTION(Category = "Savior|Data OPS", BlueprintPure, meta = (DisplayName = "[SAVIOR] Is World Partitioned", WorldContext = "Context"))
	static bool IsWorldPartitioned(UObject* Context);
	///
	/// :: UTILITY ::
	///
	...
	Target Component must contain a Boolean Property, named 'Destroyed', which is accessed via Code Reflection in
	Runtime.
	...
	//
	//
	/** Creates a new GUID from Literal String. */
	UFUNCTION(Category = "Savior", BlueprintPure, meta = (DisplayName = "[SAVIOR] Make SGUID (Input)"))
	static FGuid MakeInputSGUID(const FString Base = TEXT("AFC2254B-4E1F-F692-C270-43BAB9CA9F5B"));
	//
	/** Creates a new GUID from Literal Integer. */
	UFUNCTION(Category = "Savior", BlueprintPure, meta = (DisplayName = "[SAVIOR] Make SGUID (Literal)"))
	static FGuid MakeLiteralSGUID(const int32 A = 1111, const int32 B = 2222, const int32 C = 3333, const int32 D = 4444);
	//
  ...
  */
	...
	static USavior* NewSlotInstance(UObject* Context, USavior* Slot, ESaviorResult& Result);
	//
	...
	static USavior* ShadowCopySlot(USavior* From, USavior* To, ESaviorResult& Result);
	//
	//
	...
	static EThreadSafety GetThreadSafety()
	{
		return USavior::ThreadSafety;
	}
	//
	UFUNCTION(Category = "Savior", BlueprintCallable, meta = (DisplayName = "[SAVIOR] Get Save Progress"))
	static float GetSaveProgress()
	{
		return USavior::SS_Progress;
	}
	//
	UFUNCTION(Category = "Savior", BlueprintCallable, meta = (DisplayName = "[SAVIOR] Get Load Progress"))
	static float GetLoadProgress()
	{
		return USavior::SL_Progress;
	}
	//
	UFUNCTION(Category = "Savior", BlueprintCallable, meta = (DisplayName = "[SAVIOR] Get Saves Done"))
	static float GetSavesDone()
	{
		return USavior::SS_Complete;
	}
	//
	UFUNCTION(Category = "Savior", BlueprintCallable, meta = (DisplayName = "[SAVIOR] Get Loads Done"))
	static float GetLoadsDone()
	{
		return USavior::SL_Complete;
	}
	//
	UFUNCTION(Category = "Savior", BlueprintCallable, meta = (DisplayName = "[SAVIOR] Calculate Save Workload"))
	const float GetSaveWorkload()
	{
		return (USavior::SS_Workload = CalculateWorkload());
	}
	//
	UFUNCTION(Category = "Savior", BlueprintCallable, meta = (DisplayName = "[SAVIOR] Calculate Load Workload"))
	const float GetLoadWorkload()
	{
		return (USavior::SL_Workload = CalculateWorkload());
	}
	//
	//
	UFUNCTION(Category = "UI/UX", BlueprintCallable, meta = (CompactNodeTitle = "Set Chapter"))
	void SetChapter(FString New);
	UFUNCTION(Category = "UI/UX", BlueprintCallable, meta = (CompactNodeTitle = "Set Progress"))
	void SetProgress(float New);
	UFUNCTION(Category = "UI/UX", BlueprintCallable, meta = (CompactNodeTitle = "Set Play Time"))
	void SetPlayTime(int32 New);
	UFUNCTION(Category = "UI/UX", BlueprintCallable, meta = (CompactNodeTitle = "Set Save Date"))
	void SetSaveDate(FDateTime New);
	UFUNCTION(Category = "UI/UX", BlueprintCallable, meta = (CompactNodeTitle = "Set Player Level"))
	void SetPlayerLevel(int32 New);
	UFUNCTION(Category = "UI/UX", BlueprintCallable, meta = (CompactNodeTitle = "Set Player Name"))
	void SetPlayerName(FString New);
	UFUNCTION(Category = "UI/UX", BlueprintCallable, meta = (CompactNodeTitle = "Set Save Location"))
	void SetSaveLocation(FString New);
	//
	UFUNCTION(Category = "UI/UX", BlueprintCallable, meta = (CompactNodeTitle = "Get Chapter"))
	FString GetChapter() const;
	UFUNCTION(Category = "UI/UX", BlueprintCallable, meta = (CompactNodeTitle = "Get Progress"))
	float GetProgress() const;
	UFUNCTION(Category = "UI/UX", BlueprintCallable, meta = (CompactNodeTitle = "Get Play Time"))
	int32 GetPlayTime() const;
	UFUNCTION(Category = "UI/UX", BlueprintCallable, meta = (CompactNodeTitle = "Get Save Date"))
	FDateTime GetSaveDate() const;
	UFUNCTION(Category = "UI/UX", BlueprintCallable, meta = (CompactNodeTitle = "Get Player Name"))
	FString GetPlayerName() const;
	UFUNCTION(Category = "UI/UX", BlueprintCallable, meta = (CompactNodeTitle = "Get Player Level"))
	int32 GetPlayerLevel() const;
	UFUNCTION(Category = "UI/UX", BlueprintCallable, meta = (CompactNodeTitle = "Get Save Location"))
	FString GetSaveLocation() const;
	//
	UFUNCTION()
	void OnFinishRespawnDynamicActors(const bool Success);
};

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

UCLASS(ClassGroup = Synaptech, Category = "Synaptech")
class RTS_SURVIVAL_API USaviorSubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
public:
	UPROPERTY(Category = "Savior", BlueprintReadOnly)
	USavior* StaticSlot;
...
RTS_SURVIVAL_API bool ParseActorTAG(const AActor* Actor, const FName Tag);

/** Return True of a Component hosts an specific Tag in its Tag List. */
RTS_SURVIVAL_API bool ParseComponentTAG(const UActorComponent* Component, const FName Tag);

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// Savior Threading Utilities:

static void RespawnDynamicActors(USavior* Savior, const TMap<FName, FSaviorRecord>& SlotData)
{
	const auto&				   Settings = GetDefault<USaviorSettings>();
	TMap<FName, FSaviorRecord> ACT_Data;
	TMap<FName, FSaviorRecord> CMP_Data;
	//
	if (Settings->RespawnDynamicActors)
	{
		for (auto Pair : SlotData)
		{
			if (FPackageName::IsShortPackageName(Pair.Value.FullName))
			{
				continue;
			}
			//
		...
		for (auto Pair : ACT_Data)
		{
			FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
				...
				GET_STATID(STAT_FSimpleDelegateGraphTask_RespawnActorFromData), nullptr,
				ENamedThreads::GameThread);
		}
	}
	//
	if (Settings->RespawnDynamicComponents)
	{
		for (auto Pair : SlotData)
		{
			if (FPackageName::IsShortPackageName(Pair.Value.FullName))
		...
		for (auto Pair : CMP_Data)
		{
			FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
				...
				GET_STATID(STAT_FSimpleDelegateGraphTask_RespawnComponentFromData), nullptr,
				ENamedThreads::GameThread);
		}
	}
	//
	Savior->EVENT_OnFinishRespawn.ExecuteIfBound(!SlotData.IsEmpty());
}

class TASK_RespawnActors : public FNonAbandonableTask
{
private:
	USavior*				   Savior;
	FSavior_FinishRespawn	   OnFinishRespawn;
	TMap<FName, FSaviorRecord> RespawnData;

public:
	TASK_RespawnActors(USavior* Savior, const TMap<FName, FSaviorRecord>& SlotData)
	{
		this->Savior = Savior;
		this->RespawnData = SlotData;
	}
	//
	FORCEINLINE TStatId GetStatId() const
	{
		RETURN_QUICK_DECLARE_CYCLE_STAT(TASK_RespawnActors, STATGROUP_ThreadPoolAsyncTasks);
	}
	//
	void DoWork()
	{
		RespawnDynamicActors(Savior, RespawnData);
	}
};

class TASK_LoadGameFeatures : public FNonAbandonableTask
{
private:
	USavior* Savior;

public:
	TASK_LoadGameFeatures(USavior* Savior)
	{
		this->Savior = Savior;
	}
	//
	FORCEINLINE TStatId GetStatId() const
	{
		RETURN_QUICK_DECLARE_CYCLE_STAT(TASK_LoadGameFeatures, STATGROUP_ThreadPoolAsyncTasks);
	}
	//
	void DoWork()
	{
		ESaviorResult Result;
		Savior->LoadGameFeatures(Savior, Result);
	}
};
...
static void OnPreparedToSave()
{
	for (TObjectIterator<UObject> OBJ; OBJ; ++OBJ)
	{
		...
		if (OBJ->IsA(USaveGame::StaticClass()))
		{
			continue;
		}
		...
		if (Interface)
		{
			Interface->Execute_OnPrepareToSave(*OBJ);
		}
...
static void OnPreparedToLoad()
{
	for (TObjectIterator<UObject> OBJ; OBJ; ++OBJ)
	{
		...
		if (Interface)
		{
			Interface->Execute_OnPrepareToLoad(*OBJ);
		}
		else if ((*OBJ)->GetClass()->ImplementsInterface(USAVIOR_Serializable::StaticClass()))
		{
...
static void OnObjectSaved(UObject* Object)
{
	...
	if (Interface)
	{
		Interface->Execute_OnSaved(Object);
	}
...
static void OnAnimationSaved(UAnimInstance* Anim)
{
	...
	if (Interface)
	{
		Interface->Execute_OnSaved(Anim);
	}
...
static void OnActorSaved(AActor* Actor)
{
	...
	if (Interface)
	{
		Interface->Execute_OnSaved(Actor);
	}
...
static void OnComponentSaved(UActorComponent* Component)
{
	...
	if (Interface)
	{
		Interface->Execute_OnSaved(Component);
	}
...
static void OnGameInstanceSaved(UGameInstance* Instance)
{
	...
	if (Interface)
	{
		Interface->Execute_OnSaved(Instance);
	}
...
static void OnObjectLoaded(const FSlotMeta Meta, UObject* Object)
{
	...
	if (Interface)
	{
		Interface->Execute_OnLoaded(Object, Meta);
	}
	else if (Object->GetClass()->ImplementsInterface(USAVIOR_Serializable::StaticClass()))
	{
...
static void OnAnimationLoaded(const FSlotMeta Meta, UAnimInstance* Anim)
{
	...
	if (Interface)
	{
		Interface->Execute_OnLoaded(Anim, Meta);
	}
	else if (Anim->GetClass()->ImplementsInterface(USAVIOR_Serializable::StaticClass()))
	{
...
static void OnActorLoaded(const FSlotMeta Meta, AActor* Actor)
{
	...
	if (Interface)
	{
		Interface->Execute_OnLoaded(Actor, Meta);
	}
	else if (Actor->GetClass()->ImplementsInterface(USAVIOR_Serializable::StaticClass()))
	{
...
static void OnComponentLoaded(const FSlotMeta Meta, UActorComponent* Component)
{
	...
	if (Interface)
	{
		Interface->Execute_OnLoaded(Component, Meta);
	}
	else if (Component->GetClass()->ImplementsInterface(USAVIOR_Serializable::StaticClass()))
	{
...
static void OnGameInstanceLoaded(const FSlotMeta Meta, UGameInstance* Instance)
{
	...
	if (Interface)
	{
		Interface->Execute_OnLoaded(Instance, Meta);
	}
	else if (Instance->GetClass()->ImplementsInterface(USAVIOR_Serializable::StaticClass()))
	{
...
static void OnFinishSerializeGameWorld(USavior* Savior, ESaviorResult Result)
{
	const auto& Settings = GetDefault<USaviorSettings>();
	//
	...
	{
		if (auto PC = Savior->GetWorld()->GetFirstPlayerController())
		{
			PC->SetPause(false);
		}
	}
...
static void SerializeGameWorld(USavior* Savior)
{
	if (Savior == nullptr || !Savior->IsValidLowLevelFast() || Savior->GetWorld() == nullptr)
	{
		return;
	}
	//
	...
	//
	//
	if (USavior::IsWorldPartitioned(Savior))
	{
		Savior->SaveWorldLayers(Savior, Result);
	}
	Savior->SaveGameFeatures(Savior, Result);
	//
	for (TObjectIterator<UObject> OBJ; OBJ; ++OBJ)
	{
		if (!OBJ->IsValidLowLevelFast())
		...
		if (OBJ->GetWorld() != Savior->GetWorld())
		{
			continue;
		}
		if (OBJ->HasAnyFlags(RF_ClassDefaultObject | RF_BeginDestroyed))
		...
		//
		bool IsTarget = false;
		for (auto ACT : Savior->ActorScope)
		{
		...
		for (auto OBS : Savior->ObjectScope)
		{
		...
		for (auto COM : Savior->ComponentScope)
		{
		...
		if (IsTarget)
		{
			...
			{
				Savior->SaveComponent(Savior->GetWorld(), CastChecked<UActorComponent>(*OBJ), Result);
			}
			else if (OBJ->IsA(AActor::StaticClass()))
			{
				Savior->SaveActor(Savior->GetWorld(), CastChecked<AActor>(*OBJ), HierarchyResult);
			}
			else
			{
				Savior->SaveObjectHierarchy((*OBJ), Result, HierarchyResult);
			}
		}
	}
	//
	...
	if (IsInGameThread())
	{
		OnFinishSerializeGameWorld(Savior, Result);
	}
	else
	{
		FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
			FSimpleDelegateGraphTask::FDelegate::CreateStatic(&OnFinishSerializeGameWorld, Savior, Result),
			GET_STATID(STAT_FSimpleDelegateGraphTask_SerializeGameWorld), nullptr, ENamedThreads::GameThread);
	}
}

static void OnFinishDeserializeGameWorld(USavior* Savior, ESaviorResult Result)
{
	if (Savior == nullptr || Savior->GetWorld() == nullptr)
	{
		return;
	}
	const auto& Settings = GetDefault<USaviorSettings>();
	...
	{
		if (PlayerController->GetRootComponent())
		{
			PlayerController->GetRootComponent()->SetUsingAbsoluteRotation(true);
		}
		PlayerController->SetControlRotation(Pawn->ActorToWorld().GetRotation().Rotator());
		if (PlayerController->GetRootComponent())
		{
			PlayerController->GetRootComponent()->SetUsingAbsoluteRotation(false);
		}
	}
	//
	if (Savior->PauseGameOnLoad)
...
static void DeserializeGameWorld(USavior* Savior)
{
	if (Savior == nullptr || Savior->GetWorld() == nullptr)
	{
		return;
	...
	if (System)
	{
		USavior::ShadowCopySlot(Savior, System->StaticSlot, Result);
	}
	//
	if (USavior::IsWorldPartitioned(Savior))
	{
		Savior->LoadWorldLayers(Savior, Result);
	}
	//
	...
	for (auto Pair : DES_Data)
	{
		...
		{
			SlotData.Remove(Pair.Key);
			continue;
		}
	}
	DES_Data.Empty();
	{
		(new FAutoDeleteAsyncTask<TASK_RespawnActors>(Savior, SlotData))->StartBackgroundTask();
		(new FAutoDeleteAsyncTask<TASK_LoadGameFeatures>(Savior))->StartBackgroundTask();
	}
}

static void LoadAndActivateGameFeaturePlugin(const FFeatureRecord Feature)
{
	UGameFeaturesSubsystem::Get().LoadAndActivateGameFeaturePlugin({ Feature.PluginURL }, FGameFeaturePluginLoadComplete());
}

//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// Property Threading Interface:

static void GT_NewObjectInstance(USavior* Savior, FObjectProperty* Property, UObject* Outer, UObject* CDO,
	const FString FullName)
{
	if (!IsInGameThread() || (Outer == nullptr) || (CDO == nullptr) || (Property == nullptr))
	{
		return;
	}
	//
	...
	if (FullName.Split(TEXT("_"), nullptr, &SGUID, ESearchCase::IgnoreCase, ESearchDir::FromEnd))
	{
		...
		if (GUID.IsValid())
		{
			for (TObjectIterator<UObject> OBJ; OBJ; ++OBJ)
			{
				...
				if (OBJ->HasAnyFlags(RF_BeginDestroyed))
				{
					continue;
				}
				//
	...
	else
	{
		for (TObjectIterator<UObject> OBJ; OBJ; ++OBJ)
		{
			...
			{
				if (Reflector::MakeActorID(CastChecked<AActor>(*OBJ), true).ToString() == FullName)
				{
					Property->SetPropertyValue(ValuePtr, *OBJ);
					return;
				}
			...
			{
				if (Reflector::MakeComponentID(CastChecked<UActorComponent>(*OBJ), true).ToString() == FullName)
				{
					Property->SetPropertyValue(ValuePtr, *OBJ);
					return;
				}
	...
	//
	FString Name =
		(SGUID.IsEmpty()) ? CDO->GetName() : (CDO->GetName() + TEXT("_0")).Replace(TEXT("Default__"), TEXT(""));
...
static void GT_NewObjectIntoArray(USavior* Savior, FArrayProperty* Property, UObject* Outer, UObject* CDO,
	const FString FullName, const int32 Index)
{
	if (!IsInGameThread() || (Outer == nullptr) || (CDO == nullptr) || (Property == nullptr))
	{
		return;
	...
	//
	//
	FString Name = (SGUID.IsEmpty())
		? CDO->GetName()
		: (CDO->GetName() + TEXT("_") + FString::FromInt(Index)).Replace(TEXT("Default__"), TEXT(""));
	auto	OBJ = NewObject<UObject>(Outer, CDO->GetClass(), *Name, RF_NoFlags, CDO);
	Reflector::SetGUID(GUID, OBJ);
	//
	if (Savior)
...
static void GT_NewObjectIntoSet(USavior* Savior, FSetProperty* Property, UObject* Outer, UObject* CDO,
	const FString FullName)
{
	if (!IsInGameThread() || (Outer == nullptr) || (CDO == nullptr) || (Property == nullptr))
	{
...
static void GT_NewObjectIntoMapKey(USavior* Savior, FMapProperty* Property, UObject* Outer, UObject* CDO,
	const FString FullName)
{
	if (!IsInGameThread() || (Outer == nullptr) || (CDO == nullptr) || (Property == nullptr))
	{
...
static void GT_NewObjectIntoMapValue(USavior* Savior, FMapProperty* Property, UObject* Outer, UObject* Key,
	UObject* CDO, const FString FullName)
{
	if (!IsInGameThread() || (Outer == nullptr) || (CDO == nullptr) || (Property == nullptr))
	{
		return;
...
static void GT_NewObjectIntoNamedMapValue(USavior* Savior, FMapProperty* Property, UObject* Outer, FName Key,
	UObject* CDO, const FString FullName)
{
	if (!IsInGameThread() || (Outer == nullptr) || (CDO == nullptr) || (Property == nullptr))
	{
...
static void OnFinishSerializeLevel(USavior* Savior, ESaviorResult Result, const bool WriteFile)
{
	const auto& Settings = GetDefault<USaviorSettings>();
...
static void SerializeLevel(USavior* Savior, const FName LevelToSave, const bool WriteFile)
{
	if (Savior == nullptr || Savior->GetWorld() == nullptr || LevelToSave.IsNone())
	{
		return;
	}
	const auto&	  Settings = GetDefault<USaviorSettings>();
	...
	for (TObjectIterator<UObject> OBJ; OBJ; ++OBJ)
	{
		...
		if (!OBJ->GetOutermost()->GetName().Contains(LevelToSave.ToString(), ESearchCase::IgnoreCase))
		{
			continue;
		}
		//
		...
		if (IsTarget)
		{
			...
			{
				Savior->SaveActor(Savior->GetWorld(), CastChecked<AActor>(*OBJ), Result);
			}
			else
			{
				Savior->SaveObject((*OBJ), Result);
			}
		}
	}
	//
	...
	if (IsInGameThread())
	{
		OnFinishSerializeLevel(Savior, Result, WriteFile);
	}
	else
	{
		FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
			FSimpleDelegateGraphTask::FDelegate::CreateStatic(&OnFinishSerializeLevel, Savior, Result, WriteFile),
			GET_STATID(STAT_FSimpleDelegateGraphTask_SerializeLevel), nullptr, ENamedThreads::GameThread);
	}
}

static void OnFinishDeserializeLevel(USavior* Savior, ESaviorResult Result)
{
	check(IsInGameThread());
	//
	Savior->ClearWorkload();
...
static void DeserializeLevel(USavior* Savior, const FName LevelToLoad)
{
	if (Savior == nullptr || Savior->GetWorld() == nullptr || LevelToLoad.IsNone())
	{
		return;
	}
	const auto&	  Settings = GetDefault<USaviorSettings>();
	...
	for (TObjectIterator<UObject> OBJ; OBJ; ++OBJ)
	{
		...
		if (!OBJ->GetOutermost()->GetName().Contains(LevelToLoad.ToString(), ESearchCase::IgnoreCase))
		{
			continue;
		}
		//
		...
		if (IsTarget)
		{
			if (OBJ->IsA(UActorComponent::StaticClass()))
			{
				Savior->LoadComponent(Savior->GetWorld(), CastChecked<UActorComponent>(*OBJ), Result);
			}
			else if (OBJ->IsA(AActor::StaticClass()))
			{
	...
	if (IsInGameThread())
	{
		OnFinishDeserializeLevel(Savior, Result);
	}
	else
	{
		FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
			FSimpleDelegateGraphTask::FDelegate::CreateStatic(&OnFinishDeserializeLevel, Savior, Result),
			GET_STATID(STAT_FSimpleDelegateGraphTask_DeserializeLevel), nullptr, ENamedThreads::GameThread);
	}
}
...
static void OnFinishSerializeGameMode(USavior* Savior, ESaviorResult Result)
{
	const auto& Settings = GetDefault<USaviorSettings>();
	//
	Savior->WriteSlotToFile(Settings->DefaultPlayerID, Result);
...
static void SerializeGameMode(USavior* Savior, const bool WithGameInstance)
{
	if (Savior == nullptr || Savior->GetWorld() == nullptr)
	{
		return;
	...
	if ((PlayerController != nullptr) && (PlayerController->IsValidLowLevelFast()))
	{
		...
		//
		if ((Pawn != nullptr) && Pawn->IsValidLowLevelFast())
		{
			Savior->SaveActor(Savior->GetWorld(), Pawn, Result);
		}
		if ((PlayerHUD != nullptr) && PlayerHUD->IsValidLowLevelFast())
		{
			Savior->SaveActor(Savior->GetWorld(), PlayerHUD, Result);
		}
		if ((PlayerState != nullptr) && PlayerState->IsValidLowLevelFast())
		{
			Savior->SaveActor(Savior->GetWorld(), PlayerState, Result);
		}
	}
	//
	//
	...
	if (IsInGameThread())
	{
		OnFinishSerializeGameMode(Savior, Result);
	}
	else
	{
		FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
...
static void OnFinishDeserializeGameMode(USavior* Savior, ESaviorResult Result)
{
	const auto& Settings = GetDefault<USaviorSettings>();
	//
	Savior->ClearWorkload();
...
static void DeserializeGameMode(USavior* Savior, const bool WithGameInstance)
{
	if (Savior == nullptr || Savior->GetWorld() == nullptr)
	{
		return;
	...
	if ((PlayerController != nullptr) && PlayerController->IsValidLowLevelFast())
	{
		...
		if (Pawn != nullptr)
		{
			Savior->LoadActor(Savior->GetWorld(), Pawn, Result);
		}
		if ((PlayerHUD != nullptr) && PlayerHUD->IsValidLowLevelFast())
		{
			Savior->LoadActor(Savior->GetWorld(), PlayerHUD, Result);
		}
		if ((PlayerState != nullptr) && PlayerState->IsValidLowLevelFast())
		{
			Savior->LoadActor(Savior->GetWorld(), PlayerState, Result);
		}
	}
	//
	//
	...
	if (IsInGameThread())
	{
		OnFinishDeserializeGameMode(Savior, Result);
	}
	else
	{
		FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
...
static void OnFinishSerializeGameInstance(USavior* Savior, ESaviorResult Result)
{
	check(IsInGameThread());
	//
	const auto& Settings = GetDefault<USaviorSettings>();
...
static void SerializeGameInstance(USavior* Savior)
{
	if (Savior == nullptr || Savior->GetWorld() == nullptr)
	{
		return;
	...
	if (IsInGameThread())
	{
		OnFinishSerializeGameInstance(Savior, Result);
	}
	else
	{
		FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
			FSimpleDelegateGraphTask::FDelegate::CreateStatic(&OnFinishSerializeGameInstance, Savior, Result),
			GET_STATID(STAT_FSimpleDelegateGraphTask_SerializeGameInstance), nullptr, ENamedThreads::GameThread);
	}
}

static void OnFinishDeserializeGameInstance(USavior* Savior, ESaviorResult Result)
{
	check(IsInGameThread());
	//
	Savior->ClearWorkload();
...
static void DeserializeGameInstance(USavior* Savior)
{
	if (Savior == nullptr || Savior->GetWorld() == nullptr)
	{
		return;
	...
	if (IsInGameThread())
	{
		OnFinishDeserializeGameInstance(Savior, Result);
	}
	else
	{
		FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
			FSimpleDelegateGraphTask::FDelegate::CreateStatic(&OnFinishDeserializeGameInstance, Savior, Result),
			GET_STATID(STAT_FSimpleDelegateGraphTask_DeserializeGameInstance), nullptr, ENamedThreads::GameThread);
	}
}
...
static void DeserializeActor(const FSaviorRecord Record, AActor* Actor)
{
	...
	//
	FSoftObjectPath MeshPth = FSoftObjectPath(Record.RootMesh);
	//
	if (const auto& Avatar = Cast<ACharacter>(Actor))
	{
		if (const auto& Skel = Avatar->GetMesh())
		{
			...
			if (auto* OBJ = MeshPtr.Get())
			{
				Skel->SetSkeletalMesh(OBJ);
			}
		}
	}
	...
	if (!TAG_ANOTRAN(Actor) && !Record.IgnoreTransform && ((Scene && Scene->Mobility == EComponentMobility::Movable) || (Capsule && Capsule->Mobility == EComponentMobility::Movable)))
	{
		...
		{
			if (Reflector::MakeActorID(*PACT).ToString() == Record.OuterName)
			{
				Owner = (*PACT);
				break;
			}
		...
		{
			if (!Record.Socket.IsNone() && Owner)
			{
				Root->AttachToComponent(Owner->GetRootComponent(), FAttachmentTransformRules::SnapToTargetNotIncludingScale, Record.Socket);
			}
			else if (Owner)
			{
				Root->AttachToComponent(Owner->GetRootComponent(), FAttachmentTransformRules::KeepRelativeTransform);
			}
		}
		//
		...
		else if (Parent != nullptr && Owner == nullptr)
		{
			isRelative = false;
		}
		else if (Parent == nullptr && Owner != nullptr)
		...
		else
		{
			...
			Actor->SetActorRotation(Record.Rotation);
	...
	{
		Actor->SetActorHiddenInGame(Record.HiddenInGame);
	}
}
...
static void DeserializeComponent(const FSaviorRecord Record, UActorComponent* Component)
{
	...
	//
	if (const auto& SkelComponent = Cast<USkeletalMeshComponent>(Component))
	{
		auto MeshPtr = TSoftObjectPtr<USkeletalMesh>(MeshPth);
		if (auto* Skel = MeshPtr.Get())
		{
			SkelComponent->SetSkeletalMesh(Skel);
		}
	}
	else if (const auto& MeshComponent = Cast<UStaticMeshComponent>(Component))
	{
		auto MeshPtr = TSoftObjectPtr<UStaticMesh>(MeshPth);
		if (auto* Mesh = MeshPtr.Get())
		{
			MeshComponent->SetStaticMesh(Mesh);
		}
		//
		if (const auto& HISM = Cast<UHierarchicalInstancedStaticMeshComponent>(MeshComponent))
		{
...
static void SerializeActorMaterials(USavior* Savior, AActor* Actor)
{
	ESaviorResult Result;
	Savior->SaveActorMaterials(Actor->GetWorld(), Actor, Result);
}

static void DeserializeActorMaterials(USavior* Savior, AActor* Actor)
{
	ESaviorResult Result;
	Savior->LoadActorMaterials(Actor->GetWorld(), Actor, Result);
}
...
public:
	TASK_SerializeGameWorld(USavior* Savior)
	{
		this->Savior = Savior;
	}
	//
...
public:
	TASK_DeserializeGameWorld(USavior* Savior)
	{
		this->Savior = Savior;
	}
	//
...
public:
	TASK_SerializeLevel(USavior* Savior, const FName LevelToSave, const bool WriteFile)
	{
	...
	void DoWork()
	{
		SerializeLevel(Savior, Level, WriteSlotFile);
	}
};
...
public:
	TASK_DeserializeLevel(USavior* Savior, const FName LevelToLoad)
	{
...
public:
	TASK_SerializeGameMode(USavior* Savior, bool WithGameInstance)
	{
	...
	void DoWork()
	{
		SerializeGameMode(Savior, WithGameInstance);
	}
};
...
public:
	TASK_DeserializeGameMode(USavior* Savior, bool WithGameInstance)
	{
		this->WithGameInstance = WithGameInstance;
		this->Savior = Savior;
	}
	...
	void DoWork()
	{
		DeserializeGameMode(Savior, WithGameInstance);
	}
};
...
public:
	TASK_SerializeGameInstance(USavior* Savior)
	{
		this->Savior = Savior;
	}
	//
...
public:
	TASK_DeserializeGameInstance(USavior* Savior)
	{
		this->Savior = Savior;
	}
	//
	...
	void DoWork()
	{
		DeserializeGameInstance(Savior);
	}
};
