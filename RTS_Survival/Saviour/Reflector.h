...
RTS_SURVIVAL_API void LOG_SV(const bool Debug, const ESeverity Severity, const FName Message);
RTS_SURVIVAL_API void LOG_SV(const bool Debug, const ESeverity Severity, const TCHAR* Message);
RTS_SURVIVAL_API void LOG_SV(const bool Debug, const ESeverity Severity, const FString Message);
...
template<typename T>
bool JSONToFInstancedStruct(TSharedPtr<FJsonObject>& JSON, const FString& PID, FInstancedStruct &OutInstancedStruct)
{
	TSharedPtr<FJsonObject> JOBJ = JSON->GetObjectField(PID);
	if (!JOBJ.IsValid())
	{
		return false;
	}
	...
	T InnerStruct;
	const UStruct* StructDefinition = TBaseStructure<T>::Get();

	for (TFieldIterator<FProperty> PropIT(StructDefinition); PropIT; ++PropIT)
	{
		FProperty* Inner = *PropIT;
		const FString PropertyName = Inner->GetName();

		TSharedPtr<FJsonValue> JValue = JOBJ->TryGetField(PropertyName);
		if (JValue.IsValid())
		{
			FJsonObjectConverter::JsonValueToUProperty(JValue, Inner, Inner->ContainerPtrToValuePtr<void>(&InnerStruct), 0, 0);
		}
	}

	OutInstancedStruct.InitializeAs<T>(InnerStruct);
...
class RTS_SURVIVAL_API Reflector
{
public:
...
public:
	...
	static const FGuid ParseGUIDfromHASH(const FString& HashString);
...
public:
	...
	static bool			   RestoreMaterialFromSnapshot(UMaterialInstanceDynamic* Instance, const FString& OuterID, const FSlotData& Data);
