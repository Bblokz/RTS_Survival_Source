...
DECLARE_CYCLE_STAT(TEXT("FSimpleDelegateGraphTask.JSONToFProperty"), STAT_FSimpleDelegateGraphTask_JSONToFProperty,
	STATGROUP_TaskGraphTasks);
DECLARE_CYCLE_STAT(TEXT("FSimpleDelegateGraphTask.JSONSetToFProperty"),
	STAT_FSimpleDelegateGraphTask_JSONSetToFProperty, STATGROUP_TaskGraphTasks);
DECLARE_CYCLE_STAT(TEXT("FSimpleDelegateGraphTask.ParseJSONtoFStruct"),
	STAT_FSimpleDelegateGraphTask_ParseJSONtoFStruct, STATGROUP_TaskGraphTasks);
DECLARE_CYCLE_STAT(TEXT("FSimpleDelegateGraphTask.JSONArrayToFProperty"),
	STAT_FSimpleDelegateGraphTask_JSONArrayToFProperty, STATGROUP_TaskGraphTasks);
DECLARE_CYCLE_STAT(TEXT("FSimpleDelegateGraphTask.JSONMapKeyToFProperty"),
	STAT_FSimpleDelegateGraphTask_JSONMapKeyToFProperty, STATGROUP_TaskGraphTasks);
DECLARE_CYCLE_STAT(TEXT("FSimpleDelegateGraphTask.JSONMapValueToFProperty"),
	STAT_FSimpleDelegateGraphTask_JSONMapValueToFProperty, STATGROUP_TaskGraphTasks);
...
void LOG_SV(const bool Debug, const ESeverity Severity, const TCHAR* Message)
{
	...
	{
		case ESeverity::Info:
			UE_LOG(SaviorLog, Log, TEXT("{S}:: %s"), Message);
			break;
		case ESeverity::Warning:
			UE_LOG(SaviorLog, Warning, TEXT("{S}:: %s"), Message);
			break;
		case ESeverity::Error:
			UE_LOG(SaviorLog, Error, TEXT("{S}:: %s"), Message);
			break;
		case ESeverity::Fatal:
			UE_LOG(SaviorLog, Fatal, TEXT("{S}:: %s"), Message);
			break;
		default:
			break;
	}
...
void LOG_SV(const bool Debug, const ESeverity Severity, const FName Message)
{
	...
	{
		case ESeverity::Info:
			UE_LOG(SaviorLog, Log, TEXT("{S}:: %s"), *Message.ToString());
			break;
		case ESeverity::Warning:
			UE_LOG(SaviorLog, Warning, TEXT("{S}:: %s"), *Message.ToString());
			break;
		case ESeverity::Error:
			UE_LOG(SaviorLog, Error, TEXT("{S}:: %s"), *Message.ToString());
			break;
		case ESeverity::Fatal:
			UE_LOG(SaviorLog, Fatal, TEXT("{S}:: %s"), *Message.ToString());
			break;
		default:
			break;
	}
...
bool Reflector::IsSupportedProperty(const FProperty* Property)
{
	return
	(
		Property->IsA<FStrProperty>() || Property->IsA<FIntProperty>() ||
		Property->IsA<FSetProperty>() || Property->IsA<FMapProperty>() ||
		Property->IsA<FBoolProperty>() || Property->IsA<FByteProperty>() ||
		Property->IsA<FEnumProperty>() || Property->IsA<FNameProperty>() ||
		Property->IsA<FTextProperty>() || Property->IsA<FFloatProperty>() ||
		Property->IsA<FInt64Property>() || Property->IsA<FArrayProperty>() ||
		Property->IsA<FClassProperty>() || Property->IsA<FDoubleProperty>() ||
		Property->IsA<FStructProperty>() || Property->IsA<FObjectProperty>()
	);
}
...
const FString Reflector::SanitizeObjectPath(const FString& InPath)
{
	...
	while (*BadCH)
	{
		Sanitize.ReplaceCharInline(*BadCH, TCHAR('_'), ESearchCase::CaseSensitive);
		++BadCH;
	}
	return Sanitize;
}
...
void Reflector::RespawnActorFromData(UWorld* World, const TSet<TSubclassOf<UObject>> RespawnScope,
	const FSaviorRecord Data)
{
	...
	if (RespawnScope.Array().IsEmpty())
	{
		return;
	}
	if (Data.FullName.IsEmpty() || Data.FullName.Equals(TEXT("nullptr")))
	{
		return;
	}
	if (Data.ClassPath.IsEmpty() || Data.ClassPath.Equals(TEXT("nullptr")))
	{
		return;
	}
	//
	bool	 GUID = false;
	UObject* OTR = nullptr;
	if (Settings->AccurateDynamicRespawnChecks)
	{
		for (TActorIterator<AActor> Actor(World); Actor; ++Actor)
		{
			...
			if (Reflector::MakeActorID(*Actor, true).ToString() == Data.FullName)
			{
				GUID = true;
				break;
			}
			if (IsMatchingSGUID(Data.GUID, *Actor))
			{
				GUID = true;
				break;
			}
		...
		{
			if (Reflector::MakeObjectID(*OBJ).ToString() == Data.OuterName)
			{
				OTR = (*OBJ);
				break;
			}
	...
	if (CDO)
	{
		...
		const auto ACT = World->SpawnActorDeferred<AActor>(Class, Transform, Parameters.Owner, nullptr,
...
void Reflector::RespawnComponentFromData(UWorld* World, const TSet<TSubclassOf<UObject>> RespawnScope,
	const FSaviorRecord Data)
{
	check(IsInGameThread());
	const auto& Settings = GetDefault<USaviorSettings>();
	...
	if (Data.OuterName.IsEmpty() || Data.OuterName.Equals(TEXT("nullptr")))
	{
		return;
	}
	//
	...
	if (Settings->AccurateDynamicRespawnChecks)
	{
		...
		{
			if ((*OBJ)->HasAnyFlags(RF_DefaultSubObject | RF_BeginDestroyed))
			{
				continue;
			}
			if (Reflector::MakeComponentID(*OBJ, true).ToString() == Data.FullName)
			{
				GUID = true;
				break;
			}
			if ((Data.GUID.IsValid()) && (IsMatchingSGUID(Data.GUID, *OBJ)))
			{
				GUID = true;
				break;
			}
	...
	if (CDO)
	{
		if (!CDO->IsA(UActorComponent::StaticClass()))
		{
			return;
		}
		//
...
FMaterialRecord Reflector::CaptureMaterialSnapshot(UMaterialInstanceDynamic* Instance)
{
	...
	//
	if (Outer == nullptr || !IsValid(Outer) || !Instance->IsValidLowLevel())
	{
		return Record;
	}
	if (Instance == nullptr || !IsValid(Instance) || !Instance->IsValidLowLevel())
	{
		return Record;
	}
	//
	if (Outer->GetWorld() == nullptr)
	{
		return Record;
	}
	...
	{
		if (!ScalarInfo.IsValidIndex(I))
		{
			continue;
		}
		//
		if (!ScalarID[I].IsValid())
		{
			ScalarID[I] = ParseGUIDfromHASH(ScalarInfo[I].Name.ToString());
		}
		//
	...
	{
		if (!VectorInfo.IsValidIndex(I))
		{
			continue;
		}
		//
		if (!VectorID[I].IsValid())
		{
			VectorID[I] = ParseGUIDfromHASH(VectorInfo[I].Name.ToString());
		}
		//
	...
	{
		if (!TextureInfo.IsValidIndex(I))
		{
			continue;
		}
		//
		if (!TextureID[I].IsValid())
		{
			TextureID[I] = ParseGUIDfromHASH(TextureInfo[I].Name.ToString());
		}
		//
		...
		if (Instance->GetTextureParameterValue(ParamInfo, ParamValue))
		{
			...
			FSoftObjectPtr	  ObjPtr = FSoftObjectPtr(ParamValue);
			//
			Info.ParamName = ParamInfo.Name;
			Info.ParamValue = ObjPtr.ToSoftObjectPath();
			//
...
bool Reflector::RestoreMaterialFromSnapshot(UMaterialInstanceDynamic* Instance, const FString& OuterID,
	const FSlotData& Data)
{
	const FName ID = *FString::Printf(TEXT("%s.%s"), *OuterID, *Instance->GetName().Replace(TEXT(" "), TEXT("")));
	if (!Data.Materials.Contains(ID))
	{
		return false;
	}
	//
	...
	for (int32 I = 0; I < ScalarID.Num(); I++)
	{
		...
		if (!MatInfo.ScalarParams.Contains(ScalarID[I]))
		{
			continue;
		}
		//
		...
		if (ParamInfo.Name != ParamData.ParamName)
		{
			LOG_SV(true, ESeverity::Warning,
				FString::Printf(TEXT("Outdated Parameter (%s) Data in Storage:  \"%s\" --> \"%s\" , variable name "
									 "didn't match. Skipping..."),
					*ScalarID[I].ToString(), *ParamData.ParamName.ToString(),
	...
	for (int32 I = 0; I < VectorID.Num(); I++)
	{
		...
		if (!MatInfo.VectorParams.Contains(VectorID[I]))
		{
			continue;
		}
		//
		...
		if (ParamInfo.Name != ParamData.ParamName)
		{
			LOG_SV(true, ESeverity::Warning,
				FString::Printf(TEXT("Outdated Parameter (%s) Data in Storage:  \"%s\" --> \"%s\" , variable name "
									 "didn't match. Skipping..."),
					*VectorID[I].ToString(), *ParamData.ParamName.ToString(),
					*ParamInfo.Name.ToString()));
			continue;
		}
		//
	...
	for (int32 I = 0; I < TextureID.Num(); I++)
	{
		...
		if (!MatInfo.TextureParams.Contains(TextureID[I]))
		{
			continue;
		}
		//
		...
		if (ParamInfo.Name != ParamData.ParamName)
		{
			LOG_SV(true, ESeverity::Warning,
				FString::Printf(TEXT("Outdated Parameter (%s) Data in Storage:  \"%s\" --> \"%s\" , variable name "
									 "didn't match. Skipping..."),
					*TextureID[I].ToString(), *ParamData.ParamName.ToString(),
					*ParamInfo.Name.ToString()));
			continue;
		}
		//
		if (ParamData.ParamValue.IsValid())
		{
			...
			{
				TextureParam = Cast<UTexture>(ParamData.ParamValue.TryLoad());
			}
			if (TextureParam != nullptr)
			{
				Instance->SetTextureParameterValue(ParamInfo.Name, TextureParam);
			}
		}
	}
	//
...
TArray<TSharedPtr<FJsonValue>> Reflector::FPropertyToJSON(FArrayProperty* Property, const UObject* Container)
{
	...
	else
	{
		...
		{
			auto Value =
				FJsonObjectConverter::UPropertyToJsonValue(Property->Inner, Helper.GetRawPtr(I), 0, CPF_Transient);
...
TArray<TSharedPtr<FJsonValue>> Reflector::FPropertyToJSON(FSetProperty* Property, const UObject* Container)
{
	...
	else
	{
		for (int32 I = 0, N = Helper.Num() - 1; I <= N; I++)
		{
			auto Value = FJsonObjectConverter::UPropertyToJsonValue(Property->ElementProp, Helper.GetElementPtr(I), 0,
				CPF_Transient);
			if (Value.IsValid() && (!Value->IsNull()))
			{
				JArray.Push(Value);
...
TSharedPtr<FJsonObject> Reflector::FPropertyToJSON(FMapProperty* Property, const UObject* Container)
{
	...
	if (Property->KeyProp->IsA<FObjectProperty>())
	{
		...
		for (auto OBJ : OBJs)
		{
			if ((OBJ.Key) && OBJ.Key->IsValidLowLevelFast())
			{
				...
				{
					FullName = Reflector::MakeObjectID(OBJ.Key).ToString();
				}
				//
	...
	else
	{
		for (int32 I = 0, N = Helper.Num() - 1; I <= N; I++)
		{
			if (uint8* Ptr = Helper.GetKeyPtr(I))
			{
				auto Key = FJsonObjectConverter::UPropertyToJsonValue(Property->KeyProp, Ptr, 0, CPF_Transient);
				if (Key.IsValid() && (!Key->IsNull()))
				{
					Keys.Push(Key);
				}
			}
	...
	if ((Property->KeyProp->IsA<FNameProperty>()) && (Property->ValueProp->IsA<FObjectProperty>()))
	{
		...
		for (auto OBJ : OBJs)
		{
			if ((OBJ.Value) && OBJ.Value->IsValidLowLevelFast())
			{
				...
				{
					FullName = Reflector::MakeObjectID(OBJ.Value).ToString();
				}
				//
	...
	{
		LOG_SV(true, ESeverity::Warning,
			FString::Printf(TEXT("Property Map Value Type <-> [Data Map Value Type] mismatch:  %s"),
				*Property->GetName()));
		LOG_SV(true, ESeverity::Warning,
			TEXT("Only  TMap<UObject*,UObject*>  OR  TMap<FName,UObject*>  are supported for Maps of Object "
				 "References!"));
	}
	else
	{
		for (int32 I = 0, N = Helper.Num() - 1; I <= N; I++)
		{
			auto Value = FJsonObjectConverter::UPropertyToJsonValue(Property->ValueProp, Helper.GetValuePtr(I), 0,
				CPF_Transient);
			if (Value.IsValid() && (!Value->IsNull()))
			{
				Values.Push(Value);
...
TSharedPtr<FJsonObject> Reflector::FPropertyToJSON(FStructProperty* Property, const UObject* Container,
	const EStructType StructType)
{
	TSharedPtr<FJsonObject> JSON = MakeShareable(new FJsonObject);
	//
	switch (StructType)
	{
		case EStructType::LColor:
		{
			...
			auto Value = FJsonObjectConverter::UPropertyToJsonValue(CastFieldChecked<FProperty>(Property), StructPtr, 0,
				CPF_Transient);
			if (Value.IsValid() && (!Value->IsNull()))
			{
		...
		break;
		//
		case EStructType::Generic:
		{
			if (const FInstancedStruct* StructPtr = Property->ContainerPtrToValuePtr<FInstancedStruct>(Container))
			{
				if (const UStruct* InnerStruct = StructPtr->GetScriptStruct())
				{
					TSharedRef<FJsonObject> JsonOBJ = MakeShared<FJsonObject>();
					if (FJsonObjectConverter::UStructToJsonObject(InnerStruct, StructPtr->GetMemory(), JsonOBJ, 0, 0))
					{
						return JsonOBJ;
					}
				}
			...
			if (Property->Struct == FRuntimeFloatCurve::StaticStruct())
			{
				...
				for (TFieldIterator<FProperty> IT(Property->Struct); IT; ++IT)
				{
					...
					for (int32 I = 0; I < Field->ArrayDim; ++I)
					{
						...
						if (FStructProperty* CurveData = CastField<FStructProperty>(Field))
						{
							ParseFStructToJSON(JSON, CurveData, ValuePtr, Container);
						}
					}
				}
				break;
...
void Reflector::JSONToFProperty(TSharedPtr<FJsonObject>& JSON, const FString& PID, FBoolProperty* Property,
	UObject* Container)
{
...
void Reflector::JSONToFProperty(TSharedPtr<FJsonObject>& JSON, const FString& PID, FIntProperty* Property,
	UObject* Container)
{
	auto		ValuePtr = Property->ContainerPtrToValuePtr<void>(Container);
...
void Reflector::JSONToFProperty(TSharedPtr<FJsonObject>& JSON, const FString& PID, FInt64Property* Property,
	UObject* Container)
{
	auto		ValuePtr = Property->ContainerPtrToValuePtr<void>(Container);
...
void Reflector::JSONToFProperty(TSharedPtr<FJsonObject>& JSON, const FString& PID, FByteProperty* Property,
	UObject* Container)
{
	auto		ValuePtr = Property->ContainerPtrToValuePtr<void>(Container);
...
void Reflector::JSONToFProperty(TSharedPtr<FJsonObject>& JSON, const FString& PID, FEnumProperty* Property,
	UObject* Container)
{
...
void Reflector::JSONToFProperty(TSharedPtr<FJsonObject>& JSON, const FString& PID, FFloatProperty* Property,
	UObject* Container)
{
	auto		ValuePtr = Property->ContainerPtrToValuePtr<void>(Container);
...
void Reflector::JSONToFProperty(TSharedPtr<FJsonObject>& JSON, const FString& PID, FDoubleProperty* Property,
	UObject* Container)
{
	auto		 ValuePtr = Property->ContainerPtrToValuePtr<void>(Container);
...
void Reflector::JSONToFProperty(TSharedPtr<FJsonObject>& JSON, const FString& PID, FNameProperty* Property,
	UObject* Container)
{
	auto		ValuePtr = Property->ContainerPtrToValuePtr<void>(Container);
...
void Reflector::JSONToFProperty(TSharedPtr<FJsonObject>& JSON, const FString& PID, FTextProperty* Property,
	UObject* Container)
{
	auto		ValuePtr = Property->ContainerPtrToValuePtr<void>(Container);
...
void Reflector::JSONToFProperty(TSharedPtr<FJsonObject>& JSON, const FString& PID, FStrProperty* Property,
	UObject* Container)
{
	auto		  ValuePtr = Property->ContainerPtrToValuePtr<void>(Container);
...
void Reflector::JSONToFProperty(USavior* Savior, TSharedPtr<FJsonObject>& JSON, const FString& PID,
	FObjectProperty* Property, UObject* Container)
{
	auto	   ValuePtr = Property->ContainerPtrToValuePtr<void>(Container);
	...
	//
	if (Class.IsEmpty() || Class.Equals(TEXT("nullptr")))
	{
		return;
	}
	//
	...
	//
	auto CDO = (ClassPtr.IsNull()) ? nullptr : ClassPtr.Get();
	if ((CDO) && CDO->IsA(AActor::StaticClass()))
	{
		for (TObjectIterator<AActor> Actor; Actor; ++Actor)
		{
			if (!(*Actor)->GetOutermost()->ContainsMap() || (*Actor)->IsPendingKillPending())
			{
				continue;
			}
			if ((*Actor)->HasAnyFlags(RF_ClassDefaultObject | RF_BeginDestroyed))
			...
			{
				Property->SetPropertyValue(ValuePtr, *Actor);
				break;
			}
		}
	}
	else if ((CDO) && CDO->IsA(UActorComponent::StaticClass()))
	{
		for (TObjectIterator<UActorComponent> CMP; CMP; ++CMP)
		{
			...
			if (FullName == Instance)
			{
				Property->SetPropertyValue(ValuePtr, *CMP);
				break;
			}
		}
	}
	...
	else if ((CDO) && (Reflector::IsSafeToRespawnInstance(CDO)))
	{
		FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
			FSimpleDelegateGraphTask::FDelegate::CreateStatic(&GT_NewObjectInstance, Savior, Property, Container, CDO,
				Instance),
			GET_STATID(STAT_FSimpleDelegateGraphTask_JSONToFProperty), nullptr, ENamedThreads::GameThread);
	}
}

void Reflector::JSONToFProperty(USavior* Savior, TSharedPtr<FJsonObject>& JSON, const FString& PID,
	FClassProperty* Property, UObject* Container)
{
	auto	   ValuePtr = Property->ContainerPtrToValuePtr<void>(Container);
	...
	{
		Property->SetPropertyValue(ValuePtr, ClassPath.ResolveClass());
	}
}

void Reflector::JSONToFProperty(USavior* Savior, TSharedPtr<FJsonObject>& JSON, const FString& PID,
	FArrayProperty* Property, UObject* Container)
{
	...
	if (Property->Inner->IsA<FObjectProperty>())
	{
		...
		for (int32 I = 0, N = JArray.Num() - 1; I <= N; I++)
		{
			...
			if (JValue.IsValid() && (!JValue->AsString().IsEmpty()))
			{
				...
				{
					FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
						FSimpleDelegateGraphTask::FDelegate::CreateStatic(&GT_NewObjectIntoArray, Savior, Property,
							Container, CDO, Instance, I),
						GET_STATID(STAT_FSimpleDelegateGraphTask_JSONArrayToFProperty), nullptr,
	...
	else
	{
		for (int32 I = 0, N = JArray.Num() - 1; I <= N; I++)
		{
			...
			{
				if (!FJsonObjectConverter::JsonValueToUProperty(JValue, Property->Inner, Helper.GetRawPtr(I), 0,
						CPF_Transient))
				{
					LOG_SV(true, ESeverity::Warning,
						FString::Printf(
							TEXT("[Data Array Element Type <-> Property Array Element Type] mismatch:  %s"), *PID));
				}
			}
		}
	}
...
void Reflector::JSONToFProperty(USavior* Savior, TSharedPtr<FJsonObject>& JSON, const FString& PID,
	FSetProperty* Property, UObject* Container)
{
	...
	if (Property->ElementProp->IsA<FObjectProperty>())
	{
		...
		for (int32 I = 0, N = JSet.Num() - 1; I <= N; I++)
		{
			...
			if (JValue.IsValid() && (!JValue->AsString().IsEmpty()))
			{
				...
				{
					FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
						FSimpleDelegateGraphTask::FDelegate::CreateStatic(&GT_NewObjectIntoSet, Savior, Property,
							Container, CDO, Instance),
						GET_STATID(STAT_FSimpleDelegateGraphTask_JSONSetToFProperty), nullptr,
						ENamedThreads::GameThread);
				}
			}
		}
	...
	else
	{
		for (int32 I = 0, N = JSet.Num() - 1; I <= N; I++)
		{
			...
			if (JValue.IsValid() && (!JValue->IsNull()))
			{
				if (!FJsonObjectConverter::JsonValueToUProperty(JValue, Property->ElementProp,
						Helper.GetElementPtr(Index), 0, CPF_Transient))
				{
					LOG_SV(true, ESeverity::Warning,
						FString::Printf(TEXT("[Data Set Element Type <-> Property Set Element Type] mismatch:  %s"),
							*PID));
				}
			}
		}
...
void Reflector::JSONToFProperty(USavior* Savior, TSharedPtr<FJsonObject>& JSON, const FString& PID,
	FMapProperty* Property, UObject* Container)
{
	auto			 MapPtr = Property->ContainerPtrToValuePtr<void>(Container);
		...
		if (Property->KeyProp->IsA<FObjectProperty>())
		{
			...
			for (int32 I = 0, N = JKeys.Num() - 1; I <= N; I++)
			{
				...
				if (JKey.IsValid() && (!JKey->AsString().IsEmpty()))
				{
					...
					{
						FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
							FSimpleDelegateGraphTask::FDelegate::CreateStatic(&GT_NewObjectIntoMapKey, Savior, Property,
								Container, CDO, Instance),
							GET_STATID(STAT_FSimpleDelegateGraphTask_JSONMapKeyToFProperty), nullptr,
							ENamedThreads::GameThread);
					}
				}
			}
		...
		else
		{
			for (int32 I = 0, N = JKeys.Num() - 1; I <= N; I++)
			{
				...
				if (JKey.IsValid() && (!JKey->IsNull()))
				{
					...
					{
						LOG_SV( true, ESeverity::Warning,
							FString::Printf(TEXT("[Data Map Key Type <-> Property Map Key Type] mismatch:  %s"), *PID));
					}
				}
			}
		}
	...
	if (JMap->HasField(PID + TEXT("_VALUE")))
	{
					...
					else if ((CDO) && (Reflector::IsSafeToRespawnInstance(CDO)))
					{
						FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
							FSimpleDelegateGraphTask::FDelegate::CreateStatic(
								&GT_NewObjectIntoMapValue, Savior, Property, Container, KeyOBJ, CDO, Instance),
							GET_STATID(STAT_FSimpleDelegateGraphTask_JSONMapValueToFProperty), nullptr,
							ENamedThreads::GameThread);
					}
				}
			}
		...
		else if ((Property->KeyProp->IsA<FNameProperty>()) && (Property->ValueProp->IsA<FObjectProperty>()))
		{
			...
			for (int32 I = 0, N = JKeys.Num() - 1; I <= N; I++)
			{
				...
				if (JValue.IsValid() && (!JValue->AsString().IsEmpty()))
				{
					...
					{
						FSimpleDelegateGraphTask::CreateAndDispatchWhenReady(
							FSimpleDelegateGraphTask::FDelegate::CreateStatic(&GT_NewObjectIntoNamedMapValue, Savior,
								Property, Container, Key, CDO, Instance),
							GET_STATID(STAT_FSimpleDelegateGraphTask_JSONMapValueToFProperty), nullptr,
							ENamedThreads::GameThread);
					}
				}
		...
		else
		{
			for (int32 I = 0, N = JKeys.Num() - 1; I <= N; I++)
			{
				...
				if (JValue.IsValid() && (!JValue->IsNull()))
				{
					if (!FJsonObjectConverter::JsonValueToUProperty(JValue, Property->ValueProp, Helper.GetValuePtr(I),
							0, CPF_Transient))
					{
						LOG_SV(true, ESeverity::Warning,
							FString::Printf(TEXT("[Data Map Value Type <-> Property Map Value Type] mismatch:  %s"),
								*PID));
					}
				}
			}
...
void Reflector::JSONToFProperty(TSharedPtr<FJsonObject>& JSON, const FString& PID, FStructProperty* Property,
	UObject* Container, const EStructType StructType)
{
	switch (StructType)
	{
		case EStructType::FColor:
		case EStructType::LColor:
		{
			if (Property->Struct == TBaseStructure<FColor>::Get())
			{
				...
				if (JColor.IsValid())
				{
					Color.R = JColor->GetIntegerField(TEXT("r"));
					Color.G = JColor->GetIntegerField(TEXT("g"));
					Color.B = JColor->GetIntegerField(TEXT("b"));
					Color.A = JColor->GetIntegerField(TEXT("a"));
				}
				auto StructPtr = Property->ContainerPtrToValuePtr<FColor>(Container);
				if (StructPtr)
				{
			...
			else if (Property->Struct == TBaseStructure<FLinearColor>::Get())
			{
				...
				if (JColor.IsValid())
				{
					Color.R = JColor->GetNumberField(TEXT("r"));
					Color.G = JColor->GetNumberField(TEXT("g"));
					Color.B = JColor->GetNumberField(TEXT("b"));
					Color.A = JColor->GetNumberField(TEXT("a"));
				}
				auto StructPtr = Property->ContainerPtrToValuePtr<FLinearColor>(Container);
				if (StructPtr)
				{
			...
			{
				LOG_SV(true, ESeverity::Warning,
					FString::Printf(TEXT("[Data Type <-> Property Type] mismatch:  %s"), *PID));
			}
		}
		break;
		//
		...
		case EStructType::Generic:
		{
			...
			const auto JStruct = JSON->GetObjectField(PID);
			//
			if (FInstancedStruct* StructPtr = Property->ContainerPtrToValuePtr<FInstancedStruct>(Container))
			{
				if (const UStruct* InnerStruct = StructPtr->GetScriptStruct())
				{
					// Add any additional UStruct Type here: TBaseStructure<FMyStruct>::Get()
					if (InnerStruct == TBaseStructure<FGuid>::Get()) { DataRestored |= JSONToFInstancedStruct<FGuid>(JSON, PID, StructData); }
					if (InnerStruct == TBaseStructure<FVector>::Get()) { DataRestored |= JSONToFInstancedStruct<FVector>(JSON, PID, StructData); }
					if (InnerStruct == TBaseStructure<FVector2D>::Get()) { DataRestored |= JSONToFInstancedStruct<FVector2D>(JSON, PID, StructData); }
					if (InnerStruct == TBaseStructure<FRotator>::Get()) { DataRestored |= JSONToFInstancedStruct<FRotator>(JSON, PID, StructData); }
					if (InnerStruct == TBaseStructure<FTransform>::Get()) { DataRestored |= JSONToFInstancedStruct<FTransform>(JSON, PID, StructData); }
					if (InnerStruct == TBaseStructure<FLinearColor>::Get()) { DataRestored |= JSONToFInstancedStruct<FLinearColor>(JSON, PID, StructData); }
					if (InnerStruct == TBaseStructure<FColor>::Get()) { DataRestored |= JSONToFInstancedStruct<FColor>(JSON, PID, StructData); }
					if (InnerStruct == TBaseStructure<FVector4>::Get()) { DataRestored |= JSONToFInstancedStruct<FVector4>(JSON, PID, StructData); }
					if (InnerStruct == TBaseStructure<FQuat>::Get()) { DataRestored |= JSONToFInstancedStruct<FQuat>(JSON, PID, StructData); }
					if (InnerStruct == TBaseStructure<FMatrix>::Get()) { DataRestored |= JSONToFInstancedStruct<FMatrix>(JSON, PID, StructData); }
					if (InnerStruct == TBaseStructure<FPlane>::Get()) { DataRestored |= JSONToFInstancedStruct<FPlane>(JSON, PID, StructData); }
					if (InnerStruct == TBaseStructure<FSphere>::Get()) { DataRestored |= JSONToFInstancedStruct<FSphere>(JSON, PID, StructData); }
					if (InnerStruct == TBaseStructure<FBox2D>::Get()) { DataRestored |= JSONToFInstancedStruct<FBox2D>(JSON, PID, StructData); }
					if (InnerStruct == TBaseStructure<FRay>::Get()) { DataRestored |= JSONToFInstancedStruct<FRay>(JSON, PID, StructData); }
					//
					if (DataRestored)
					{
						(*StructPtr) = StructData;
					}
				}
			}
		}
...
TSharedPtr<FJsonObject> Reflector::ParseFStructToJSON(FStructProperty* Property, const void* StructPtr,
...
void Reflector::ParseFStructToJSON(TSharedPtr<FJsonObject>& JSON, FStructProperty* Property, const void* StructPtr,
	const UObject* Container)
{
	UScriptStruct* Struct = Property->Struct;
	//
...
void Reflector::ParseJSONtoFStruct(TSharedPtr<FJsonObject>& JSON, FStructProperty* Property, void* StructPtr,
	UObject* Container)
{
	UScriptStruct* Struct = Property->Struct;
	//
	for (TFieldIterator<FProperty> IT(Struct); IT; ++IT)
	{
		...
		for (int32 I = 0; I < Field->ArrayDim; ++I)
		{
			...
			if (ValuePtr)
			{
				ParseJSONtoFStructProperty(JSON, Field, ValuePtr, Container, 0, CPF_Transient, Struct->IsNative());
			}
		}
	}
}

void Reflector::ParseJSONtoFStructProperty(TSharedPtr<FJsonObject>& JSON, FProperty* Property, void* ValuePtr,
	UObject* Container, int64 CheckFlags, int64 SkipFlags, const bool IsNative)
{
	...
	if (FArrayProperty* _Array = CastField<FArrayProperty>(Property))
	{
		...
		if (JSON->HasField(Field))
		{
			...
			{
				/// Can't go so deep. Unreal Engine has a deep check to prevent us from serializing any Array of Object
				/// Pointers, Members of User-Defined Structs, so this is disabled:
				LOG_SV(true, ESeverity::Warning,
					FString::Printf(
						TEXT("Unreal Engine explicitly disallows serializing ASCII Array of Object References (when "
							 "members of User-Defined Structs) so this Struct Member will NOT be loaded:  %s"),
						*Field));
			}
			else
			{
				...
				for (int32 I = 0, N = JArray.Num() - 1; I <= N; I++)
				{
					...
					if (JValue.IsValid() && (!JValue->IsNull()))
					{
						if (!FJsonObjectConverter::JsonValueToUProperty(JValue, _Array->Inner, Helper.GetRawPtr(I), 0,
								SkipFlags))
						{
							LOG_SV(true, ESeverity::Warning,
								FString::Printf(
									TEXT("[Data Array Element Type <-> Property Array Element Type] mismatch:  %s"),
									*Field));
						}
					}
				}
	...
	if (FSetProperty* _Set = CastField<FSetProperty>(Property))
	{
		...
		if (JSON->HasField(Field))
		{
			...
			{
				/// Can't go so deep. Unreal Engine has a deep check to prevent us from serializing any Set of Object
				/// Pointers, Members of User-Defined Structs, so this is disabled:
				LOG_SV(true, ESeverity::Warning,
					FString::Printf(
						TEXT("Unreal Engine explicitly disallows serializing ASCII Set of Object References (when "
							 "members of User-Defined Structs) so this Struct Member will NOT be loaded:  %s"),
						*Field));
			}
			else
			{
				...
				for (int32 I = 0, N = JSet.Num() - 1; I <= N; I++)
				{
					...
					if (JValue.IsValid() && (!JValue->IsNull()))
					{
						if (!FJsonObjectConverter::JsonValueToUProperty(JValue, _Set->ElementProp,
								Helper.GetElementPtr(Index), 0, SkipFlags))
						{
							LOG_SV(true, ESeverity::Warning,
								FString::Printf(
									TEXT("[Data Set Element Type <-> Property Set Element Type] mismatch:  %s"),
									*Field));
						}
					}
				}
	...
	if (FMapProperty* _Map = CastField<FMapProperty>(Property))
	{
		...
		if (JSON->HasField(Field + TEXT("_KEY")))
		{
			...
			{
				/// Can't go so deep. Unreal Engine has a deep check to prevent us from serializing any Map of Object
				/// Pointers, Members of User-Defined Structs, so this is disabled:
				LOG_SV(true, ESeverity::Warning,
					FString::Printf(
						TEXT("Unreal Engine explicitly disallows serializing ASCII Map of Object References (when "
							 "members of User-Defined Structs) so this Struct Member will NOT be loaded:  %s"),
						*Field));
			}
			else
			{
				...
				for (int32 I = 0, N = JKeys.Num() - 1; I <= N; I++)
				{
					...
					if (JKey.IsValid() && (!JKey->IsNull()))
					{
						if (!FJsonObjectConverter::JsonValueToUProperty(JKey, _Map->KeyProp, Helper.GetKeyPtr(IKey), 0,
								SkipFlags))
						{
							LOG_SV(true, ESeverity::Warning,
								FString::Printf(TEXT("[Data Map Key Type <-> Property Map Key Type] mismatch:  %s"),
									*Field));
						}
					}
				}
		...
		if (JSON->HasField(Field + TEXT("_VALUE")))
		{
			...
			else
			{
				...
				for (int32 I = 0, N = JValues.Num() - 1; I <= N; I++)
				{
					...
					if (JValue.IsValid() && (!JValue->IsNull()))
					{
						if (!FJsonObjectConverter::JsonValueToUProperty(JValue, _Map->ValueProp, Helper.GetValuePtr(I),
								0, SkipFlags))
						{
							LOG_SV(
	...
	if (FStructProperty* _Struct = CastField<FStructProperty>(Property))
	{
		...
		if (JSON->HasField(Field))
		{
			...
			if (JStruct.IsValid())
			{
				ParseJSONtoFStruct(JStruct, _Struct, ValuePtr, Container);
			}
		}
	}
	//
	if (FObjectProperty* _Object = CastField<FObjectProperty>(Property))
	{
		...
		if (JSON->HasField(Field))
		{
			/// Can't go so deep. Unreal Engine has a deep check to prevent us from serializing any Object Pointers,
			/// Members of User-Defined Structs, so this is disabled:
			LOG_SV(
				true, ESeverity::Warning,
				FString::Printf(TEXT("Unreal Engine explicitly disallows serializing ASCII Object References (when "
									 "members of User-Defined Structs) so this Struct Member will NOT be loaded:  %s"),
					*Field));
		}
		else
		{
			LOG_SV(true, ESeverity::Warning,
				FString::Printf(TEXT("[Data Object Struct Member <-> Property Object Struct Member] mismatch:  %s"),
					*Field));
		}
	}
	//
	if (FClassProperty* _Class = CastField<FClassProperty>(Property))
	{
		...
		{
			LOG_SV(true, ESeverity::Warning,
				FString::Printf(TEXT("[Data Class Struct Member <-> Property Class Struct Member] mismatch:  %s"),
					*Field));
		}
	}
}
...
void Reflector::ParseFStructPropertyToJSON(TSharedPtr<FJsonObject>& JSON, FProperty* Property, const void* ValuePtr,
	const UObject* Container, int64 CheckFlags, int64 SkipFlags,
	const bool IsNative)
{
	...
	if (FEnumProperty* _Enum = CastField<FEnumProperty>(Property))
	{
		UEnum*	EnumPtr = _Enum->GetEnum();
		FString Value =
			EnumPtr->GetNameStringByValue(_Enum->GetUnderlyingProperty()->GetUnsignedIntPropertyValue(ValuePtr));
	...
	if (FArrayProperty* _Array = CastField<FArrayProperty>(Property))
	{
		...
		if (_Array->Inner->IsA<FObjectProperty>() || _Array->Inner->IsA<FClassProperty>())
		{
			...
			LOG_SV(true, ESeverity::Warning,
				FString::Printf(
					TEXT("Unreal Engine explicitly disallows serializing ASCII Array of Object References (when "
						 "members of User-Defined Structs) so this Struct Member will NOT be saved:  %s"),
					*_Array->GetName()));
		}
		else
		{
			for (int32 I = 0, N = Helper.Num() - 1; I <= N; I++)
			{
				auto Value = FJsonObjectConverter::UPropertyToJsonValue(_Array->Inner, Helper.GetRawPtr(I), CheckFlags,
					SkipFlags);
				if (Value.IsValid() && (!Value->IsNull()))
				{
					JArray.Push(Value);
	...
	if (FSetProperty* _Set = CastField<FSetProperty>(Property))
	{
		...
		else
		{
			for (int32 I = 0, N = Helper.Num() - 1; I <= N; I++)
			{
				auto Value = FJsonObjectConverter::UPropertyToJsonValue(_Set->ElementProp, Helper.GetElementPtr(I),
	...
	if (FMapProperty* _Map = CastField<FMapProperty>(Property))
	{
		...
		else
		{
			for (int32 I = 0, N = Helper.Num() - 1; I <= N; I++)
			{
				if (uint8* Ptr = Helper.GetKeyPtr(I))
				{
					auto Key = FJsonObjectConverter::UPropertyToJsonValue(_Map->KeyProp, Ptr, CheckFlags, SkipFlags);
					if (Key.IsValid() && (!Key->IsNull()))
					{
						Keys.Push(Key);
					}
		...
		if (_Map->ValueProp->IsA<FObjectProperty>() || _Map->ValueProp->IsA<FClassProperty>())
		{
			...
			LOG_SV(true, ESeverity::Warning,
				FString::Printf(
					TEXT("Unreal Engine explicitly disallows serializing ASCII Map of Object References (when "
						 "members of 'User-Defined Structs') so this Struct Member will NOT be saved:  %s"),
					*_Map->ValueProp->GetName()));
		}
		else
		{
			for (int32 I = 0, N = Helper.Num() - 1; I <= N; I++)
			{
				auto Value = FJsonObjectConverter::UPropertyToJsonValue(_Map->ValueProp, Helper.GetValuePtr(I),
					CheckFlags, SkipFlags);
				if (Value.IsValid() && (!Value->IsNull()))
				{
					Values.Push(Value);
	...
	if (FObjectProperty* _Object = CastField<FObjectProperty>(Property))
	{
		/// Can't go so deep. Unreal Engine has a deep check to prevent us from serializing any Object Pointers, Members
		/// of User-Defined Structs, so this is disabled:
		LOG_SV(true, ESeverity::Warning,
			FString::Printf(TEXT("Unreal Engine explicitly disallows serializing ASCII Object References (when "
								 "members of User-Defined Structs) so this Struct Member will NOT be saved:  %s"),
				*_Object->GetName()));
...
bool Reflector::HasGUID(UObject* Container)
{
	...
	//
	for (TFieldIterator<FProperty> PIT(Container->GetClass(), EFieldIteratorFlags::IncludeSuper,
		...
		PIT; ++PIT)
	{
		...
		if ((IsStruct) && (CastFieldChecked<FStructProperty>(Property)->Struct == TBaseStructure<FGuid>::Get()) && (Property->GetName().Equals(TEXT("SGUID"), ESearchCase::CaseSensitive)))
		{
			return true;
		}
	}
...
const FGuid Reflector::ParseGUID(FStructProperty* Property, const UObject* Container)
{
	...
	const auto	 Value =
		FJsonObjectConverter::UPropertyToJsonValue(CastFieldChecked<FProperty>(Property), StructPtr, 0, 0);
	//
...
const FGuid Reflector::ParseGUIDfromHASH(const FString& HashString)
{
	uint32		  iA = 0, iB = 0, iC = 0, iD = 0;
	const FString Hash = FMD5::HashAnsiString(*HashString);
	{
		FString A{}, B{}, C{}, D{};
...
{
	if (!Target->IsValidLowLevelFast() || !ComparedTo->IsValidLowLevelFast())
	{
		return false;
	}
	FGuid GUID;
	...
	OID.Invalidate();
	//
	for (TFieldIterator<FProperty> PIT(Target->GetClass(), EFieldIteratorFlags::IncludeSuper,
			 EFieldIteratorFlags::ExcludeDeprecated, EFieldIteratorFlags::ExcludeInterfaces);
		PIT; ++PIT)
	{
		FProperty* Property = *PIT;
	...
	//
	for (TFieldIterator<FProperty> PIT(ComparedTo->GetClass(), EFieldIteratorFlags::IncludeSuper,
			 EFieldIteratorFlags::ExcludeDeprecated, EFieldIteratorFlags::ExcludeInterfaces);
		PIT; ++PIT)
	{
		FProperty* Property = *PIT;
...
bool Reflector::GetPlayerNetworkID(const APlayerController* PlayerController, FString& ID, const bool AppendPort)
{
	...
	if (!PlayerController->PlayerState->IsValidLowLevelFast())
	{
		return false;
	}
	...
	if ((!ID.IsEmpty()) && (!AppendPort))
	{
		ID.Split(FString(":"), &ID, nullptr, ESearchCase::IgnoreCase, ESearchDir::FromEnd);
	}
	//
	return false;
}
...
const FName Reflector::MakeObjectID(UObject* OBJ, bool FullPath)
{
	...
	{
		LOG_SV(true, ESeverity::Warning,
			FString::Printf(TEXT("Cannot create Object ID for invalid object reference.")));
	...
	if (AActor* const& ACTO = Cast<AActor>(Owner))
	{
		...
		{
			if (!GetPlayerNetworkID(ACTO->GetNetOwningPlayer()->GetPlayerController(ACTO->GetWorld()), NetworkID,
					false))
			{
			...
			{
				ID = (NetworkID + FString(TEXT("_")) + ID);
			}
		}
	}
	else if (UActorComponent* const& CMP_Outer = Cast<UActorComponent>(Owner))
	{
		if (AActor* const& ACTC = CMP_Outer->GetOwner())
		{
			...
			{
				if (!GetPlayerNetworkID(ACTC->GetNetOwningPlayer()->GetPlayerController(ACTC->GetWorld()), NetworkID,
						false))
				{
...
const FName Reflector::MakeComponentID(UActorComponent* CMP, bool FullPath)
{
	...
	{
		LOG_SV(true, ESeverity::Warning,
			FString::Printf(TEXT("Cannot create Component ID for invalid component reference.")));
		return TEXT("ERROR:ID");
	}
	//
	FString		OwnerID;
