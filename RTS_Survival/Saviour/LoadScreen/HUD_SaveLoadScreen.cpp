...
void AHUD_SaviorUI::DisplaySplashLoadScreenHUD(const EThreadSafety TaskMode, const FText FeedbackText, const FSlateFontInfo& FeedbackFont, const FLinearColor& ProgressBarTint, const FSoftObjectPath SplashAsset, TEnumAsByte<EStretch::Type> SplashStretch)
{
	if (GEngine == nullptr || GEngine->GameViewport == nullptr)
	{
		return;
...
void AHUD_SaviorUI::DisplayMovieLoadScreenHUD(const EThreadSafety TaskMode, const FText FeedbackText, const FSlateFontInfo& FeedbackFont, const FLinearColor& ProgressBarTint, const FSoftObjectPath SplashMovie, const bool ProgressBarOnMovie)
{
	if (GEngine == nullptr || GEngine->GameViewport == nullptr)
	{
		return;
...
{
	if ((USavior::GetSaveProgress() >= 100.f) && (USavior::GetLoadProgress() >= 100.f))
	{
...
void AHUD_SaviorUI::StaticRemoveLoadScreen()
{
	if (LoadScreenBlurWidget.IsValid())
	{
		LoadScreenBlurWidget->SetVisibility(EVisibility::Collapsed);
	}
	if (LoadScreenMovieWidget.IsValid())
	{
		LoadScreenMovieWidget->SetVisibility(EVisibility::Collapsed);
	}
	if (LoadScreenSplashWidget.IsValid())
	{
		LoadScreenSplashWidget->SetVisibility(EVisibility::Collapsed);
	}
}
