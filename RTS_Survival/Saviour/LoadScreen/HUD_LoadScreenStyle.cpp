...
const ISlateStyle& FSaviorLoadScreenStyle::IGet()
{
	return StyleSet.ToSharedRef().Get();
}
TSharedPtr<ISlateStyle> FSaviorLoadScreenStyle::Get()
{
	return StyleSet;
}
...
void FSaviorLoadScreenStyle::Initialize()
{
	...
	//
	{
		StyleSet->Set("SaviorProgressBar", FProgressBarStyle().SetBackgroundImage(BOX_BRUSH("Common/GroupBorder", FMargin(5.f / 12.f))).SetFillImage(BOX_BRUSH("Common/ProgressBar_Fill", FMargin(5.f / 12.f), FLinearColor::White)).SetMarqueeImage(IMAGE_BRUSH("Common/ProgressBar_Marquee", FVector2D(20, 12), FLinearColor::White, ESlateBrushTileType::Horizontal)));
	}
	//
