...
#include "RTS_Survival/Saviour/Savior.h"
#include "RTS_Survival/Saviour/SaviorTypes.h"
...
void HUD_LoadScreenMovieWidget::Construct(const FArguments& InArgs)
{
	...
	HUD_Overlay->AddSlot(10000)
		...
		.HAlign(HAlign_Fill)
			[SNew(SSafeZone)
					...
					.VAlign(VAlign_Bottom)
						[SNew(SDPIScaler)
								.DPIScale(this, &HUD_LoadScreenMovieWidget::GetDPIScale)
									[SNew(SVerticalBox)
...
void HUD_LoadScreenMovieWidget::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
	...
	if (InCurrentTime > Ticker)
	{
		...
		if (FeedbackText.ToString().Contains(TEXT("...")))
		{
			FeedbackText = FText::FromString(FeedbackText.ToString().Replace(TEXT("..."), TEXT(".  ")));
		}
		else if (FeedbackText.ToString().Contains(TEXT(".. ")))
		{
			FeedbackText = FText::FromString(FeedbackText.ToString().Replace(TEXT(".. "), TEXT("...")));
		}
		else if (FeedbackText.ToString().Contains(TEXT(".  ")))
...
float HUD_LoadScreenMovieWidget::GetDPIScale() const
{
	return DPIScale;
}

TOptional<float> HUD_LoadScreenMovieWidget::GetWorkloadProgress() const
{
	switch (TaskMode)
	{
...
void HUD_LoadScreenMovieWidget::SetProgressBarTint(const FLinearColor& NewColor)
{
	ProgressBarTint = NewColor;
}

void HUD_LoadScreenMovieWidget::SetFeedBackFont(const FSlateFontInfo& NewFont)
{
	FeedbackFont = NewFont;
}

void HUD_LoadScreenMovieWidget::SetFeedbackText(const FString& NewText)
{
	FeedbackText = FText::FromString(NewText);
}

void HUD_LoadScreenMovieWidget::SetTaskMode(const EThreadSafety NewMode)
{
	TaskMode = NewMode;
}
