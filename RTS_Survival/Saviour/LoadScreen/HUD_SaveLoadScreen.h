...
#include "RTS_Survival/Saviour/SlotScreen/HUD_SlotPicker.h"
...
class RTS_SURVIVAL_API AHUD_SaviorUI : public AHUD
{
	GENERATED_BODY()
	//
	friend class USavior;
...
private:
	UPROPERTY()
	USlotPickerWidget* SlotPickerUI;
...
public:
	...
	void DisplaySplashLoadScreenHUD(const EThreadSafety TaskMode, const FText FeedbackText, const FSlateFontInfo& FeedbackFont, const FLinearColor& ProgressBarTint, const FSoftObjectPath SplashAsset, TEnumAsByte<EStretch::Type> SplashStretch);
	//
	/** Forces a loading screen canvas to draw on HUD. */
	UFUNCTION(Category = "Savior", BlueprintCallable, meta = (DisplayName = "[SAVIOR] Invoke Load Screen (Video)"))
	void DisplayMovieLoadScreenHUD(const EThreadSafety TaskMode, const FText FeedbackText, const FSlateFontInfo& FeedbackFont, const FLinearColor& ProgressBarTint, const FSoftObjectPath SplashMovie, const bool ProgressBarOnMovie);
	//
